...
public:
	...
	OpenFile(int f, int type) {		// open file with type
		file = f; 
		currentOffset = 0; 
		this->type = type; 
	}
...
public:
	...
	OpenFile(int sector, int type);	// Open file with type
	...
	int ReadAt(char *into, int numBytes, int position);		// Read/write bytes from the file,
															// bypassing the implicit position.
	int WriteAt(char *from, int numBytes, int position);

	int Length(); 			// Return the number of bytes in the file (this interface is simpler 
							// than the UNIX idiom -- lseek to end of file, tell, lseek back 
